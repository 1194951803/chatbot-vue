...
用户点击"员工自助"按钮 → 按钮变蓝 → mode → employee_self
    ↓ AI 发送问候文本（提示可用功能）
    ↓ 用户输入自然语言，如"我想明天上午请假"
AI 调用后端 /ai/api/intent/employee（POST JSON Body，SSE 流式响应）
    ↓ 前端复用 createStreamRequest 逐块拼接 data.output.text，实时展示模型回复
    ↓ 每帧中提取 data.output.session_id，流结束时保存
    ↓ finish_reason === "stop" 时流结束
    ↓ 检查是否有 tool_calls 判断是否匹配意图
    ↓ 有 tool_calls → 从 function.arguments 提取参数 → 渲染对应卡片
    ↓ 无 tool_calls → 展示模型追问文本，等待用户继续输入
    ↓ 用户继续回复 → 回传 sessionId → 后端累积上下文 → 再次识别
    ↓ 匹配成功后渲染卡片
    ↓ 用户点击"确认提交"
AI 发送提交成功文本消息
    ↓ 用户再次点击"员工自助"按钮 → 按钮恢复白色
本地清空 sessionId → mode → customer_service + 系统提示退出消息
```

**关键实现**：

- **快捷按钮 Toggle**（`Chatbot.vue` `handleQuickAction`）：当前已是该模式时退出到 `CUSTOMER_SERVICE`，否则进入该模式
- **模式监听**（`watch` on `modeStore.currentMode`）：进入 `EMPLOYEE_SELF` 时发送问候消息；退出时清空本地 sessionId
- **SSE 流式响应**：复用 `createStreamRequest` 处理 SSE，实时展示 `data.output.choices[0].message.content` 增量文本；每帧提取 `data.output.session_id`，流结束时保存到 `chatStore.employeeSessionId`
- **意图判断**：`extractIntentFromToolCalls()` 仅从 SSE 帧中的 `tool_calls` 字段提取意图和参数（Function Calling 模式），无 `tool_calls` 则视为模型追问，展示文本回复
- **多轮对话**：`chatStore` 维护 `employeeSessionId`，首次调用不传，后端返回后保存，后续请求回传 `sessionId` 参数实现上下文连贯；上下文持久化到 `ai_chat_records` 表
...
| 员工自助多轮对话 | 通过 `sessionId` 参数维护上下文，首次不传后端返回 ID，后续回传实现追问补全；退出时清空本地状态 |
